 ...
 * {ERC721Enumerable}.
 */
contract wassim is Context, ERC165, IERC721  {
    ...
    mapping(uint256 => uint256) private _royaltyPercents;
    ...
    function mint(address to, address royaltyRecipient, uint256 royaltyPercent) public {
    ...
    function royaltyInfo(uint256 tokenId) public view returns (address receiver, uint256 royaltyAmount) {
        require(_owners[tokenId] != address(0), "Token does not exist");
        receiver = _royaltyRecipients[tokenId];
        royaltyAmount = (salePrice * _royaltyPercents[tokenId]) / 100;
